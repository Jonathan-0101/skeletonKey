...
// void SubGHzTools::init(fs::SDFS &sdInstance) {
void SubGHzTools::init() {
  ...
  digitalWrite(CC1101_B_RF_Switch, HIGH);
...
void SubGHzTools::captureRaw(int sampleInterval = 1) {
  ...
  for (int i = 0; i < RECORDINGBUFFERSIZE; i++) {
    ...
    {
      bitWrite(receivedbyte, j, digitalRead(CC1101_B_gdo0));  // Capture GDO0 state into the byte
      delayMicroseconds(setting);                             // delay for selected sampling interval
    };
    // store the output into recording buffer
    CC1101_B_bigrecordingbuffer[i] = receivedbyte;
  }
  ...
  for (int i = 0; i < RECORDINGBUFFERSIZE; i++) {
    Serial.print(CC1101_B_bigrecordingbuffer[i], HEX);  // Print each byte in hexadecimal format
    Serial.print(" ");
  }
}
...
void SubGHzTools::transmitRaw(int sampleInterval = 1) {
  ...
  pinMode(CC1101_B_gdo0, OUTPUT);

  for (int i = 1; i < RECORDINGBUFFERSIZE; i++) {
    byte receivedbyte = CC1101_B_bigrecordingbuffer[i];
    for (int j = 7; j > -1; j--)  // 8 bits in a byte
    {
      digitalWrite(CC1101_B_gdo0, bitRead(receivedbyte, j));  // Set GDO0 according to recorded byte
      delayMicroseconds(setting);                             // delay for selected sampling interval
    };
  }
